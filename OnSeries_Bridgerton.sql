/*
OBJECTIVE: BRIDGERTON SERIES BY JULIA QUINN
...
VALUES		('Julia Quinn', 'Bridgerton', 8, 'On The Way To The Wedding', 496, 'Gregory', 'M', 26, 'Lucy Abernathy', 'F', 20)
...
----- DATA ON PRICES ARE BASED ON AMAZON.DE AND AUDIBLE.DE.
...
SELECT		Book_Title AS Most_Expensive, CONCAT('€', Hardcover_Price) AS Hardcover_Price
FROM		#bridgerton_prices
WHERE		Hardcover_Price = (SELECT MAX(Hardcover_Price) FROM #bridgerton_prices)

SELECT		Book_Title AS Least_Expensive, CONCAT('€', Hardcover_Price) AS Hardcover_Price
FROM		#bridgerton_prices
WHERE		Hardcover_Price = (SELECT MIN(Hardcover_Price) FROM #bridgerton_prices)


----- PAPERBACK.

SELECT		CONCAT('€', Paperback_Price) AS Higest_To_Lowest, Book_Order AS '№', Book_Title
FROM		#bridgerton_prices
ORDER BY	Higest_To_Lowest DESC

SELECT		AVG(Paperback_Pages) Paperback_Average_Page, CONCAT('€', ROUND(AVG(Paperback_Price), 2)) Paperback_Average_Price
FROM		#bridgerton_prices


----- KINDLE.

SELECT		(Kindle_Price/Kindle_Pages) Price_Per_Page, Book_Title Kindle_Format, CONCAT(Kindle_Pages, ' pages for €', Kindle_Price) Best_Values
FROM		#bridgerton_prices
ORDER BY	Price_Per_Page

SELECT		Book_Title, Hardcover_Pages, Kindle_Pages, CONCAT((100-(100*Hardcover_Pages/Kindle_Pages)), '%') Size_Change
FROM		#bridgerton_prices
ORDER BY	Book_Order
...
SELECT		Book_Title, CONCAT(Audio_Length,' Minutes') Audio_Length, CONCAT(Audio_Length/60, ' Hours') Approximately
FROM		#bridgerton_prices
ORDER BY	Book_Order

SELECT		Book_Order, Book_Title, CONCAT(Audio_Length,' Minutes') Audio_Length, (Audible_Price/Audio_Length) Price_Per_Minute, 
		CONCAT('€', Audible_Price) Best_Value
FROM		#bridgerton_prices
ORDER BY	Best_Value
